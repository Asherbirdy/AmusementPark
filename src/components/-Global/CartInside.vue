...
// 在適當的地方觸發該事件（例如，從購物車中新增或刪除商品時）
const updateCartItemCount = () => {
  // 計算購物車內商品的總數量
  let total = 0;
  displayTicketData.value.forEach(item => {
    total += item.count;
  });
  // 觸發事件，通知父元件更新購物車數量
  emit('cart-item-count-updated', total);
};

onMounted(() => {
  // 監聽購物車數量更新事件，並觸發相應的回調函式
  const cartItemCountUpdatedListener = (count) => {
    cartItemCount.value = count;
  };

  // 監聽父元件發送的購物車數量更新事件
  // 使用合適的事件名稱替換'cart-item-count-updated'
  // 使用合適的父元件替換'parent-component'
  const parentComponent = getParentComponent(); // 替換為實際的父元件
  parentComponent.$on('cart-item-count-updated', cartItemCountUpdatedListener);
});
